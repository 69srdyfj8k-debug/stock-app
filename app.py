...
if df is None or df.empty or len(df) < 2:
    ...
    with tab1:
        ...
        st.write(f"**{t['fair_val']}** ${fair_value:.2f} \vert{} **{t['max_buy']}**${max_buy_price:.2f}")
        ...
        if buy_cost > 0:
            ...
            st.write(f"**{t['stop_price_label']}**: ${stop_loss_price:.2f} \vert{} **{t['target_sell_label']}**:${target_sell_price:.2f}")

            with st.expander(t["view_sell_reasons"], expanded=True):
                for sr in sell_reasons:
                    st.write(sr)

    # --- 🥈 Tab 2：📊 總覽與市場新聞 (含 AI 助手) ---
    with tab2:
        ...
        col1.metric(t["latest_price"], f"${latest_close收到！這代表第一個分頁（First Tab）的名稱是 **💡 估值與持股診斷**。

請問第二個分頁（Second Tab）的名稱是什麼？或者你需要我幫忙設計這兩個 Tab 對應的內容架構或前端 UI 程式碼？        "news_header": "📰 實時新聞 Grouping 與智能市場結語 (Conclusion)",
        "no_news": "暫無最新新聞數據。",
        "assistant_title": "💬 智能 AI 分析助手",
        "chat_placeholder": "輸入你想了解的關鍵字，例如：支持位、RSI、止損、現價 等。",
        "chat_hint": "你可以試下問：`現價`、`支持位`、`止損` 或 `新聞` ！",
...
請問第二個分頁（Second Tab）的名稱是什麼？或者你需要我幫忙設計這兩個 Tab 對應的內容架構或前端 UI 程式碼？        "assistant_title": "💬 智能 AI 分析助手",
        "chat_placeholder": "輸入你想了解的關鍵字，例如：支持位、RSI、止損、現價 等。",
        "chat_hint": "你可以試下問：`現價`、`支持位`、`止損` 或 `新聞` ！",
        "data_error": "無法抓取數據，請檢查股票代號是否正確，或嘗試換個時間週期（Timeframe）。",
        "cat_earnings": "💰 業績與財務數據",
...
if df is None or df.empty or len(df) < 2:
    ...
    with tab1:
        ...
        col1, col2, col3, col4 = st.columns(4)
        col1.metric(t["latest_price"], f"${latest_close:.2f}", f"{price_change:+.2f} ({pct_change:+.2f}%)")
