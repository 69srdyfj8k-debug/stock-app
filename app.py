...
if page == "📊 總覽與 AI 決策":
    ...
    if not df.empty:
        ...
        if latest_close > ma20_val > ma50_val:
            score += 2
            reasons.append("✅ **多頭排列**：股價高於 20MA 及 50MA，處於上升通道。")
        ...
        if latest_rsi > 70:
            score -= 1
            reasons.append("⚠️ **RSI 超買 (>70)**：技術面短期過熱，追高風險較大。")
        elif latest_rsi < 30:
            score += 1
        ...
        if score >= 2:
            st.success(
                f"#### 🟢 **建議：【偏多／考慮分批入場】**\n\n"
                f"**操作策略**：目前技術面偏強，若想建倉，建議採用 **分批買入** 策略。\n\n"
        ...
        if user_prompt := st.chat_input("想問關於這隻股票的什麼問題？（例如：止損要設在哪裡？）："):
            ...
            with st.chat_message("assistant"):
                response = (
                    ...
                    f"建議結合上方 AI 操盤手給出的建議，將 20MA 或 50MA 當作主要參考指標。"
                )
                st.write(response)
...
# ==========================================
    ...
    chart_df = stock.history(period=selected_config["period"], interval=selected_config["interval"])
    ...
    if not chart_df.empty:
        ...
        x_axis = chart_df.index.strftime('%Y-%m-%d %H:%M') if 'm' in selected_config['interval'] or 'h' in selected_config['interval'] else chart_df.index.strftime('%Y-%m-%d')

        fig.add_trace(go.Candlestick(x=x_axis, open=chart_df['Open'], high=chart_df['High'], low=chart_df['Low'], close=chart_df['Close'], name='K線'), row=1, col=1)
        fig.add_trace(go.Scatter(x=x_axis, y=chart_df['MA20'], mode='lines', name='20MA', line=dict(color='orange', width=1.5)), row=1, col=1)
        fig.add_trace(go.Scatter(x=x_axis, y=chart_df['MA50'], mode='lines', name='50MA', line=dict(color='blue', width=1.5)), row=1, col=1)

        fig.add_trace(go.Scatter(x=x_axis, y=chart_df['RSI'], mode='lines', name='RSI', line=dict(color='purple', width=1.5)), row=2, col=1)
